...
export default function AIAssistantUI() {
    ...
    const pinned = filtered.filter((c) => c.pinned).sort((a, b) => (new Date(a.updatedAt) < new Date(b.updatedAt) ? 1 : -1))
    ...
    const folderCounts = React.useMemo(() => {
        const map: Record<string, number> = Object.fromEntries(folders.map((f) => [f.name, 0]))
        for (const c of conversations) if (c.folder && map[c.folder] != null) map[c.folder] += 1
        return map
    ...
    function createFolder(nameInput?: string) {
        const name = nameInput || prompt("Folder name")
        if (!name) return
